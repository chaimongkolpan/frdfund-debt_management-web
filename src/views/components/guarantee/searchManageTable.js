...
const SearchTable = (props) => {
  const { result, filter, getData, handleShowDetail, handlePlan, handleAsset, handleGuarantor,  handleReturnGuarantee, handleSpouse, handleSubmit, handleUpload, handleEdit, handleReturn, can_action
        ...
        <div className="d-flex align-items-center justify-content-center my-3">
          <div className={`${isSome ? '' : 'd-none'}`}>
            <div className="d-flex">
              <button className="btn btn-warning btn-sm ms-2" type="button" onClick={() => onEdit()}>แก้ไขโอนหลักทรัพย์ (บริหารสินทรัพย์)</button>
              <button className="btn btn-primary btn-sm ms-2" type="button" onClick={() => onSubmit()}>จัดเก็บหลักทรัพย์</button>
            </div>
          </div>
        </div>
      )}
