    ...
    const map_petitions = data.map(item => {
      return {
        ...
        amount: cheques?.length > 0 ? 
          (findCheque(0) ? item.debt_manage_outstanding_principal_remain ?? item.debt_manage_outstanding_principal : 0)
          + (findCheque(1) ? item.debt_manage_accrued_interest_remain ?? item.debt_manage_accrued_interest : 0)
          + (findCheque(2) ? item.debt_manage_fine_remain ?? item.debt_manage_fine : 0)
          + (findCheque(3) ? item.debt_manage_litigation_expenses_remain ?? item.debt_manage_litigation_expenses : 0)
          + (findCheque(4) ? item.debt_manage_forfeiture_withdrawal_fee_remain ?? item.debt_manage_forfeiture_withdrawal_fee : 0)
    ...
    const map_petitions = data.map(item => {
      return {
        ...
        amount: branchCheck?.length > 0 ? 
          (branchCheck[0] ? item.debt_manage_outstanding_principal_remain ?? item.debt_manage_outstanding_principal : 0)
          + (branchCheck[1] ? item.debt_manage_accrued_interest_remain ?? item.debt_manage_accrued_interest : 0)
          + (branchCheck[2] ? item.debt_manage_fine_remain ?? item.debt_manage_fine : 0)
          + (branchCheck[3] ? item.debt_manage_litigation_expenses_remain ?? item.debt_manage_litigation_expenses : 0)
          + (branchCheck[4] ? item.debt_manage_forfeiture_withdrawal_fee_remain ?? item.debt_manage_forfeiture_withdrawal_fee : 0)
          + (branchCheck[5] ? item.debt_manage_insurance_premium_remain ?? item.debt_manage_insurance_premium : 0)
          + (branchCheck[6] ? item.debt_manage_other_expenses_remain ?? item.debt_manage_other_expenses : 0)
        : item.debt_manage_total_remain ?? item.debt_manage_total,
        pay_debt_manage_outstanding_principal: (branchCheck[0] ? item.debt_manage_outstanding_principal_remain ?? item.debt_manage_outstanding_principal : 0),
        pay_debt_manage_accrued_interest: (branchCheck[1] ? item.debt_manage_accrued_interest_remain ?? item.debt_manage_accrued_interest : 0),
        pay_debt_manage_fine: (branchCheck[2] ? item.debt_manage_fine_remain ?? item.debt_manage_fine : 0),
        pay_debt_manage_litigation_expenses: (branchCheck[3] ? item.debt_manage_litigation_expenses_remain ?? item.debt_manage_litigation_expenses : 0),
        pay_debt_manage_forfeiture_withdrawal_fee: (branchCheck[4] ? item.debt_manage_forfeiture_withdrawal_fee_remain ?? item.debt_manage_forfeiture_withdrawal_fee : 0),
        pay_debt_manage_insurance_premium: (branchCheck[5] ? item.debt_manage_insurance_premium_remain ?? item.debt_manage_insurance_premium : 0),
        pay_debt_manage_other_expenses: (branchCheck[6] ? item.debt_manage_other_expenses_remain ?? item.debt_manage_other_expenses : 0),
      }
    })
    const t_cheque = [{
        debt_management_type,
                          ...
                          <div className="col-sm-12 col-md-12 col-lg-12 mb-2" >
                            <div className="table-responsive mx-n1 px-1">
                              <div className="d-flex flex-column">
                                ...
                                <div className="d-flex flex-wrap">
                                  <div className="me-3"><input className="form-check-input" type="checkbox" checked={branchCheck[0]} onChange={() => BranchChange(0)}/> &nbsp;เงินต้น</div>
                                  <div className="me-3"><input className="form-check-input" type="checkbox" checked={branchCheck[1]} onChange={() => BranchChange(1)}/> &nbsp;ดอกเบี้ย</div>
