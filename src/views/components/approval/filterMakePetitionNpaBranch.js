...
const Filter = (props) => {
  ...
  const onSubmit = () => {
    if (handleSubmit) {
      handleSubmit({
        ...
        creditor: "",
        debtStatus: "",
        debtClassifyStatusList: ['โอนเงินให้สาขาแล้ว'],
        ...filter,
        currentPage: 1,
  ...
  const onChange = async (key, val) => {
    ...
    if (key == 'proposal_committee_no') {
      ...
      const resultCommitteeDate = await getCommitteeDateNpa("\'โอนเงินให้สาขาแล้ว\'", val);
  ...
  async function fetchData() {
    ...
    const resultCommitteeNo = await getCommitteeNoNpa("\'โอนเงินให้สาขาแล้ว\'");
    ...
    if (resultCommitteeNo.isSuccess) {
      ...
      const resultCommitteeDate = await getCommitteeDateNpa("\'โอนเงินให้สาขาแล้ว\'", temp[0]);
      if (resultCommitteeDate.isSuccess) {
        const temp = resultCommitteeDate.data.map(item => item.name);
        await setCommitteeDateOp(temp);
        await setFilter((prevState) => ({
