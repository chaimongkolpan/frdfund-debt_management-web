...
const Filter = (props) => {
  ...
  const [statusDebtOp, setStatusDebtOp] = useState(["รอรับคืนเงิน","รับคืนเงินแล้ว"]);
  ...
  const onChange = async (key, val) => {
    ...
    if (key == 'book_no') {
      ...
      const resultCommitteeDate = await getReturnPetitionDate("\'รอรับคืนเงิน\',\'รับคืนเงินแล้ว\'", val);
  ...
  async function fetchData() {
    ...
    const resultCommitteeNo = await getReturnPetitionNo("\'รอรับคืนเงิน\',\'รับคืนเงินแล้ว\'");
    ...
    if (resultCommitteeNo.isSuccess) {
      ...
      const resultCommitteeDate = await getReturnPetitionDate("\'รอรับคืนเงิน\',\'รับคืนเงินแล้ว\'", temp[0]);
      if (resultCommitteeDate.isSuccess) {
        const temp = resultCommitteeDate.data.map(item => item.name);
        await setCommitteeDateOp(temp);
        await setFilter((prevState) => ({
    ...
    <>
      <form className="row g-3">
        ...
        <div className="col-sm-12 col-md-6 col-lg-6">
          {committeeNoOp && (
            <Dropdown 
              title={'เลขที่หนังสือสาขา'} 
              defaultValue={committeeNoOp[0]} 
              options={committeeNoOp}
              handleChange={(val) => onChange('book_no', val)}
            />
          )}
        </div>
        <div className="col-sm-12 col-md-6 col-lg-6">
          {committeeDateOp && (
            <Dropdown 
              title={'วันที่หนังสือสาขา'} 
              defaultValue={committeeDateOp[0]} 
              options={committeeDateOp}
              handleChange={(val) => onChange('book_date', val)}
            />
          )}
        </div>
        <div className="col-sm-12 col-md-6 col-lg-6">
