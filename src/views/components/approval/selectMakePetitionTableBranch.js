...
const SelectedTable = (props) => {
  ...
  const onChange = async (id) => {
    ...
    const sum = selectedData.reduce((prev, item) => { return prev + (item.debt_manage_total-(item.debt_manage_total_remain ?? 0)); }, 0)
  ...
  const onHeaderChange = async (checked) => {
    ...
    if (checked) {
      ...
      const sum = result.data.reduce((prev, item) => { return prev + (item.debt_manage_total-(item.debt_manage_total_remain ?? 0)); }, 0)
      await setCount(custs.length.toLocaleString());
