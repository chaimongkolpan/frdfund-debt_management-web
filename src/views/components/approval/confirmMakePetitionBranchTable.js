  ...
  const onSave = () => {
    ...
    const map_petitions = data.map(item => {
      return {
        ...
        pay_debt_manage_outstanding_principal: (findCheque(0) ? item.debt_manage_outstanding_principal : 0),
        pay_debt_manage_accrued_interest: (findCheque(1) ? item.debt_manage_accrued_interest : 0),
        pay_debt_manage_fine: (findCheque(2) ? item.debt_manage_fine : 0),
        pay_debt_manage_litigation_expenses: (findCheque(3) ? item.debt_manage_litigation_expenses : 0),
        pay_debt_manage_forfeiture_withdrawal_fee: (findCheque(4) ? item.debt_manage_forfeiture_withdrawal_fee : 0),
        pay_debt_manage_insurance_premium: (findCheque(5) ? item.debt_manage_insurance_premium : 0),
        pay_debt_manage_other_expenses: (findCheque(6) ? item.debt_manage_other_expenses : 0),
      }
    })
