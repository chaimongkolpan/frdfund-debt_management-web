...
const SearchTable = (props) => {
  const { result, filter, getData, handleShowDetail, handlePlan, handleAsset, handleGuarantor, handleBorrower, handleSpouse, handleSubmit, handleUpload, handleEdit, handleReturn, can_action
    , handleViewEdit, handleViewEditAsset, handleViewReturn, handleShowFarmerDetail, handleShowCard, handleBorrowerPolicy
  } = props;
        ...
        <td className="align-middle white-space-nowrap text-center pe-0">
          <div className="btn-reveal-trigger position-static">
            ...
            <div className="dropdown-menu dropdown-menu-end py-2">
              ...
              <button className="dropdown-item" type="button" onClick={() => handleBorrowerPolicy(item)}>เพิ่มบุคคลรับสภาพหนี้แทน (ชั่วคราว)</button>
          ...
          <table className="table table-sm table-striped table-bordered fs-9 mb-0">
            <thead className="align-middle text-center text-nowrap" style={{ backgroundColor: '#d9fbd0',border: '#cdd0c7' }}>
              <tr>
                ...
                <th colSpan="2">หนังสือนำส่งจัดการหนี้</th>
        ...
        <div className="d-flex align-items-center justify-content-center my-3">
          <div className={`${isSome ? '' : 'd-none'}`}>
            <div className="d-flex">
              <button className="btn btn-warning btn-sm ms-2" type="button" onClick={() => onEdit()}>แก้ไขนิติกรรมสัญญา (บริหารสินทรัพย์)</button>
              <button className="btn btn-primary btn-sm ms-2" type="button" onClick={() => onSubmit()}>จัดเก็บนิติกรรมสัญญา</button>
            </div>
          </div>
        </div>
      )}
