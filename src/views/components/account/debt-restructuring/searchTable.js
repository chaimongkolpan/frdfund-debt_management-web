...
const SearchTable = (props) => {
  const { result, filter, getData, handleUpload
  } = props;
  ...
  const RenderData = (item, index) => {
    return (item && (
      <tr key={index}>
        ...
        <td>{item.restructurePolicyStatus}</td>
        <td className="align-middle white-space-nowrap text-center pe-0">
          <div className="d-flex justify-content-center"> 
            <button className="btn btn-phoenix-secondary btn-icon fs-7 text-success-dark px-0" onClick={() => handleUpload(item)}><i className="fas fa-plus-square"></i></button>
          </div>
        </td>
      </tr>
    ))
          ...
          <table className="table table-sm table-striped table-bordered fs-9 mb-0">
            <thead className="align-middle text-center text-nowrap" style={{ backgroundColor: '#d9fbd0',border: '#cdd0c7' }}>
              <tr>
                ...
                <th rowSpan="2">สถานนะรับสภาพหนี้</th>
