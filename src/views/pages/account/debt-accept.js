...
const PageContent = () => {
  ...
  const onSubmitFile = async () => {
    if (files && files.length > 0) {
      ...
      form.append('document_type', 'เอกสารรับสภาพบังคับ');
      form.append('id_RestructurePolicy', policy.id_RestructurePolicy);
      form.append('RestructurePolicyStatus', 'รับสภาพบังคับแล้ว');
  ...
  const onSearch = async (filter) => {
    setLoadBigData(true);
    setFilter(filter)
    const result = await searchDebtAccept(filter);
    if (result.isSuccess) {
      setData(result)
    } else {
      setData(null)
  ...
  return (
    <>
      <div className="content">
        <h4 className="mb-3">หนังสือรับสภาพบังคับ</h4>
        <div className="mt-4">
