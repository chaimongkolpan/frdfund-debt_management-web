...
import Filter from "@views/components/account/debt-acknowledge/filter";
import SearchTable from "@views/components/account/debt-acknowledge/searchTable";
...
const PageContent = () => {
  ...
  const onSearch = async (filter) => {
    setLoadBigData(true);
    setFilter(filter)
    const result = await searchDebtAcknowledge(filter);
    if (result.isSuccess) {
      setData(result)
    } else {
      setData(null)
  ...
  return (
    <>
      <div className="content">
        <h4 className="mb-3">หนังสือรับสภาพหนี้</h4>
        <div className="mt-4">
      ...
      {openUpload && (
        <Modal isOpen={openUpload} setModal={setOpenUpload} hideOk onClose={() => handleCloseUpload()}  title={`อัพโหลดเอกสารประกอบรับสภาพหนี้ นิติกรรมสัญญาเลขที่ ${policy?.policyNO}`} closeText={'ปิด'} scrollable size={'xl'}>
          <form>
            <br />
            <div className="row">
              {oldfiles && (
