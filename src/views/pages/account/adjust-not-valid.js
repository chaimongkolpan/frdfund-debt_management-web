...
import Filter from "@views/components/account/adjust/filterNotValid";
import SearchTable from "@views/components/account/adjust/searchTableNotValid";
import Textbox from "@views/components/input/Textbox";
import DatePicker from "@views/components/input/DatePicker";
import DropZone from "@views/components/input/DropZone";
import { 
  cleanData,
  searchAdjustNotValid,
} from "@services/api";
...
const PageContent = () => {
  ...
  const onSearch = async (filter) => {
    setLoadBigData(true);
    setFilter(filter)
    const result = await searchAdjustNotValid(filter);
    if (result.isSuccess) {
      setData(result)
    } else {
      setData(null)
  ...
  return (
    <>
      <div className="content">
        <h4 className="mb-3">เงินไม่เข้าบัญชีกองทุน</h4>
        <div className="mt-4">
