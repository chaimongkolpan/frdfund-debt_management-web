...
import Filter from "@views/components/account/operation-land/filterRental";
import SearchTable from "@views/components/account/operation-land/searchTableRental";
...
const PageContent = () => {
  ...
  const onSearch = async (filter) => {
    setLoadBigData(true);
    setFilter(filter)
    const result = await searchRental(filter);
    if (result.isSuccess) {
      setData(result)
    } else {
      setData(null)
