...
const LegalContractSend = () => {
  ...
  const onSubmitReturn = async () => {
    const param = selectedData.map(item => {
      return {
        ...
        branch_policy_no: 'กฟก ' + getBookNoReturn() + bookNo,
        branch_policy_date: ToDateDb(bookDate),
      }
    });
    const result = await submitSendLegal(param);
  ...
  return (
    <>
      ...
      {openUpload && (
        <Modal isOpen={openUpload} setModal={setOpenUpload} hideOk onClose={() => handleCloseUpload()}  title={`ดาวน์โหลดเอกสารนิติกรรมสัญญาเลขที่ ${policy?.policyNO}`} closeText={'ปิด'} scrollable size={'xl'}>
          <form>
            <br />
